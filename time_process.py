...
df = pd.read_csv('code/data/NYC_train.csv')
poi_df = pd.read_csv('code/data/poi_info.csv')
...
df.to_csv('code/data/NYC_train.csv', index=False)

df = pd.read_csv('code/data/NYC_val.csv')
for i in range(df.shape[0]):
    df.loc[i, 'time_period'] = time2idx_dic[int(df.loc[i, 'UTC_time'][11:13]) * (60 / size_time_slices) + int(int(df.loc[i, 'UTC_time'][14:16]) / size_time_slices)]
df.to_csv('code/data/NYC_val.csv', index=False)

df = pd.read_csv('code/data/NYC_test.csv')
for i in range(df.shape[0]):
    df.loc[i, 'time_period'] = time2idx_dic[int(df.loc[i, 'UTC_time'][11:13]) * (60 / size_time_slices) + int(int(df.loc[i, 'UTC_time'][14:16]) / size_time_slices)]
df.to_csv('code/data/NYC_test.csv', index=False)
